...
class ZhihuParser:
    def __init__(self, cookies, hexo_uploader=False, keep_logs=False):
        ...
        self.user_agents = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36"
        self.headers = {
        ...
        if self.keep_logs:
            ...
            # 只加文件handler，不重复加console handler
    ...
    def log(self, level, message):
        """自定义日志函数，只在keep_logs为True时记录，并直接print到控制台"""
        if self.keep_logs:
            if level == 'info':
        ...
        print(f'[ZhihuParser][{level.upper()}] {message}')
    ...
    def check_connect_error(self, target_link):
        ...
        try:
            ...
            print(f"[ZhihuParser][ERROR] HTTP error occurred: {err}")
            raise
        except requests.exceptions.RequestException as err:
            print(f"[ZhihuParser][ERROR] Error occurred: {err}")
            raise

        self.soup = BeautifulSoup(response.content, "html.parser")
        if self.soup.text.find("有问题，就会有答案打开知乎App在「我的页」右上角打开扫一扫其他扫码方式") != -1:
            print("[ZhihuParser][WARNING] Cookies are required to access the article.")
            raise ValueError("Cookies are required to access the article.")

        if self.soup.text.find("你似乎来到了没有知识存在的荒原") != -1:
            print("[ZhihuParser][WARNING] The page does not exist.")
            raise ValueError("The page does not exist.")
        
        if self.soup.text.find("安全验证") != -1:
            print("[ZhihuParser][WARNING] The page validate.")
            raise ValueError("The page validate.")
    ...
    def parse_zhihu_answer(self, target_link):
        """
        解析知乎回答并保存为 Markdown 格式文件，并返回点赞数
        """
        print(f'[parse_zhihu_answer] start: {target_link}')
        try:
            self.check_connect_error(target_link)
            print('[parse_zhihu_answer] after check_connect_error')
            # 找到回答标题、内容、作者所在的元素
            # print(f'[parse_zhihu_answer] soup: {self.soup}')
            title_element = self.soup.select_one("h1.QuestionHeader-title")
            print(f'[parse_zhihu_answer] title_element: {title_element}')
            content_element = self.soup.select_one("div.RichContent-inner")
            date = get_article_date(self.soup, "div.ContentItem-time")
            print(f'[parse_zhihu_answer] date: {date}')
            author = self.soup.select_one('div.AuthorInfo').find(
                'meta', {'itemprop': 'name'}).get('content')
            print(f'[parse_zhihu_answer] author: {author}')
            # 获取点赞数
            vote_count = 0
            vote_meta = self.soup.select_one('meta[itemprop="upvoteCount"]')
            print(f'[parse_zhihu_answer] vote_meta: {vote_meta}')
            if vote_meta and vote_meta.has_attr('content'):
                try:
                    vote_count = int(vote_meta['content'])
                except Exception:
                    vote_count = vote_meta['content']
            print(f'[parse_zhihu_answer] vote_count: {vote_count}')
            # 解析知乎文章并保存为Markdown格式文件
            markdown_title = self.save_and_transform(
                title_element, content_element, author, target_link, date)
            print(f'[parse_zhihu_answer] markdown_title: {markdown_title}')
            origin_title = title_element.text if title_element else ''
            print(f'[parse_zhihu_answer] origin_title: {origin_title}')
            self.log('info', f"Successfully parsed answer: {markdown_title}, 赞同数: {vote_count}")
            return markdown_title, vote_count,origin_title
        except Exception as e:
            print(f'[parse_zhihu_answer] error: {e}')
